...
| `flow/torch_ppo_train.py` | `Walker2d-v5` | 同一 flow policy 下的标准 PPO 对照实现 |

## 使用 `main.py` 并行启动实验

`main.py` 是一个多进程启动器。它会为每个子进程设置独立的
`CUDA_VISIBLE_DEVICES`，并将 `--` 后面的参数透传给训练入口；训练模块会在
子进程中单独加载，因此不同 GPU 和随机种子可以同时运行。

例如，在两张 GPU 上启动两个不同 seed 的 ExO-PPO 实验：
...
启动 PPO 对照实验时，将 `--algorithm flow` 改成 `--algorithm ppo` 即可：

```bash
python main.py --algorithm ppo --gpus 0,1 --seeds 0,1 --max-parallel 2 -- \
  --env-id Walker2d-v5 --env-backend gymnasium --device auto \
  --total-steps 1000000 --num-envs 16 --rollout-steps 256 \
  --replay-N 1 --warmup-rollouts 1
```

`--gpus 1` 会让子进程只看到物理 GPU 1；因此通常使用
`--device auto` 或省略 `--device`，不要再传 `--device cuda:1`。启动器会自动
为每个任务隔离日志目录，例如 `logs/launcher/flow-seed0-gpu0-run0/`。

如果每个实验需要不同的环境、算法或超参数，可以使用 JSON 配置文件：
...
```bash
python main.py --config runs.json
```

使用 `--dry-run` 可以只打印最终命令而不启动训练；使用
`--keep-going` 可以在某个实验失败后继续运行其他实验。
...
- 训练模块尚未提供统一的 `requirements.txt`；训练参数仍由各入口的 CLI 或脚本配置控制。
