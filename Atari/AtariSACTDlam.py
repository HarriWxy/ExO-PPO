...
class Memory:
    def __init__(self, obs_shape):
        ...
        self.actions = np.zeros((self.size+1,1),dtype=np.int32)
    ...
    def compute_gae(self, cri_net:Critic_val):
        obs = tf.convert_to_tensor(self.obses,tf.float32)
        values = cri_net(obs)
        values = tf.gather_nd(values,self.actions,batch_dims=1).numpy()
        ...
        if  t > OBSERVE and istrain : # 
            ...
            for i in np.random.permutation(Train_step):
                ...
                for minibatch in sample_inde():
                    ...
                    loss1 = train_critic1(b_r,b_s,b_a)
...
def train_actor(b_s):
    with tf.GradientTape() as tape: 
        ...
        dq1 = alpha*log_pis - critic_val1(b_s)
...
if __name__ == "__main__":
    ...
    critic_val1 = Critic_val(action_dim)
    log_alpha = tf.Variable(0.0)
    alpha = tf.math.exp(log_alpha)
    target_entropy = 0.4#-np.prod(action_dim)

    optimizer_alp = optimizers.Adam(learning_rate = 1e-4)
    optimizer_cri = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = 1e-5))
