...
'on-policy exo for ablation study'
...
GAMMA = 0.990 # 未来奖励的衰减
Replay_N = 1 # 8# 观测存储器D的容量
BATCH = 256 # 训练batch大小
Lam_Gae = 0.95
Run_Step = 256
Train_Env_num = 8 #8 // Replay_N 
Test_Env_num = 5
EPS = 24800 * 2 // Train_Env_num 
Rec_Num = 32 * 8 // Train_Env_num #
Train_step = 4  # 8:4
...
def trainNet(istrain):
    ...
    # tensorboard
    train_log_dir='logs/'+ENV_NAME+'/'+datetime.datetime.now().strftime("%m%d-%H%M%S")+'onex'+str(seed)
    train_sum_writer = tf.summary.create_file_writer(train_log_dir)

    eps = 0
    reward = np.zeros(Train_Env_num,np.float32)
    while eps < EPS:
        # stage = int(np.floor(eps*4/EPS))
        # beta = beta + stage*2
        # Replay_N = Replay_N - stage*2
        ...
        # D_mem.extendleft(cp.deepcopy(mem))
        ...
        if  t > OBSERVE and istrain : # 
            ...
            for i in range(Train_step):
                ...
                for minibatch in sample_inde():
                    ...
                    # b_gae = tf.convert_to_tensor(b_gae,dtype=tf.float32)  #等会儿看一下那边是怎么解决这个问题的k
...
def train_actor(b_s,b_a,b_ra,b_gae):
    with tf.GradientTape() as tape: 
        ...
        dq2 = tf.minimum(tf.multiply(dq1, b_gae),tf.multiply(dq2,b_gae))
...
if __name__ == "__main__":
    seed = 0    # 0 42 1024 42
    # for seed in seeds:
    env =  envpool.make_gymnasium(ENV_NAME,num_envs=Train_Env_num,seed=seed, episodic_life=True, reward_clip=True)# gym.make(ENV_NAME)
    eva_env = envpool.make_gymnasium(ENV_NAME,num_envs=Test_Env_num,seed=seed, reward_clip=False) # 
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(5e-4,8000,0.985,staircase=True)
    optimizer_ac = optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-5)
    lr_schedual_ = optimizers.schedules.ExponentialDecay(2e-4,8000,0.985,staircase=True)
