...
def trainNet(istrain):
    ...
    # tensorboard
    train_log_dir='logs/'+ENV_NAME+'/'+datetime.datetime.now().strftime("%m%d-%H%M%S")+'exo-value'+str(seed)
...
def train_actor(b_s,b_a,b_ra,b_gae):
    with tf.GradientTape() as tape: 
        ...
        dq2 = tf.multiply(dq1, b_gae) # clip  # tf.minimum(tf.multiply(dq2, b_gae),clip_adv)
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(3e-4,8000,0.985,staircase=True)
    optimizer_ac = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-5))
    lr_schedual_ = optimizers.schedules.ExponentialDecay(2.5e-4,8000,0.985,staircase=True)
    optimizer_cri = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual_,epsilon=1e-5))
    trainNet(True)
