...
'value clip, adv clip, adv norm, Early stop ppo + kl'

os.environ['CUDA_VISIBLE_DEVICES']='0'
ENV_NAME = 'LeaperEasy-v0'
...
if __name__ == "__main__":
    seed = 0    #  [0,0] 
    # for seed in seeds:
    env =  envpool.make_gymnasium(ENV_NAME, num_envs=Train_Env_num,seed=seed)# gym.make(ENV_NAME)episodic_life=True,reward_clip=True,
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(2.8e-3,8000,0.98,staircase=True)
    optimizer_ac = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-5))
    lr_schedual_ = optimizers.schedules.ExponentialDecay(2e-3,8000,0.98,staircase=True)
    optimizer_cri = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual_,epsilon=1e-5))

    trainNet(True)
        # del env,eva_env,actor_val,critic_val,optimizer_alp,optimizer_ac,optimizer_cri
