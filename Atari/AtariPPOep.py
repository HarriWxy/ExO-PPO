...
def trainNet(istrain):
    ...
    if vit:
        vit_path = "_vit"
    train_log_dir='logs/'+ENV_NAME+vit_path+'/'+datetime.datetime.now().strftime("%m%d-%H%M%S")+'PPO'+str(seed)
...
def train_actor(b_s,b_a,b_ra,b_gae):
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        dq2 = tf.minimum(tf.multiply(dq1, b_gae),clip_adv) # clip
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(3.5e-4,5000,0.99,staircase=True)
    optimizer_ac = optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-8)
    lr_schedual_ = optimizers.schedules.ExponentialDecay(3e-4,5000,0.98,staircase=True)
