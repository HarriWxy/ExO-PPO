...
' Base: Entropy -  pi * V * pi/beta  // V -> reward ,no clip + kl'
...
#============================ 训练网络 ===========================================
        ...
        if  t > OBSERVE and istrain :
            ...
            for i in range(40):
                ...
                b_ra = tf.convert_to_tensor([d[4] for d in minibatch],dtype=tf.float32)
                b_gae = tf.convert_to_tensor([d[5] for d in minibatch],dtype=tf.float32)
                ...
                # if i == temp_t-1:
            ep_reward = evaluate()
            print("ep=",eps,"loss1 = %f" % loss1,"ac-loss = %f" % ac_loss,"alpha= %f" % alpha,"score=",ep_reward)
            ...
            if  ep_best < ep_reward or ep_reward > 200:
                ...
                # critic_tar1.set_weights(critic_val2.get_weights())
                # actor_val.save_wei()
                # actor_tar.save_wei()
...
def train_actor(b_s,b_a,b_ra,b_gae):
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        dq2 = tf.multiply(dq2, b_gae) # clip
...
def train_kl(b_s):
    with tf.GradientTape() as tape:
        a_temp = actor_val(b_s)
        kloss = losses.kl_divergence(actor_tar(b_s),a_temp)
        kloss = tf.reduce_mean(kloss)
        kloss = optimizer_ac4.get_scaled_loss(kloss)
    gradients = tape.gradient(kloss,actor_val.trainable_variables)
    gradients = optimizer_ac4.get_unscaled_gradients(gradients)
    optimizer_ac4.apply_gradients(zip(gradients, actor_val.trainable_variables))
    return kloss
...
if __name__ == "__main__":
    ...
    optimizer_ac2 = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = 1e-3))
    optimizer_ac3 = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = 1e-4))
    optimizer_ac4 = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = 1e-5))
