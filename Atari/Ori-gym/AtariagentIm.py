...
from keras.layers import  Dense, Activation,LayerNormalization,Conv2D,Flatten,AveragePooling2D,MaxPool2D
# import os 

class ImagePro(Model):
    def __init__(self):
        super().__init__() 
        # resnet
        self.c_1_1 = Conv2D(filters=32, kernel_size=8,strides=4, padding='valid',activation='elu',input_shape=(210,160,3),
                            kernel_initializer='he_uniform')
        self.p_1 = MaxPool2D(pool_size=(2, 2), strides=2, padding='valid')  # 池化层
        self.l_1 =LayerNormalization(-1)
        self.c_2_1 = Conv2D(filters=32, kernel_size=4,strides=2, padding='valid',activation='elu',
                            kernel_initializer='he_uniform')
        self.c_2_2 = Conv2D(filters=32, kernel_size=4,strides=1, padding='same',kernel_initializer='he_uniform')
        ...
        self.c_3_1 = Conv2D(filters=64, kernel_size=3,strides=1, padding='valid',activation='elu',
                            kernel_initializer='he_uniform')
        self.l_4 = Flatten()
...
class Actor_val(Model): 
    # 评估网络,输出动作
    def __init__(self, actions, imnet:ImagePro):
        super().__init__() 
        # resnet
        # self.f1 = Dense(256, activation='elu',kernel_initializer='he_uniform')
        self.ima_net = imnet
        self.f4 = Dense(64, activation='elu',kernel_initializer='he_uniform')
        self.f41 = Dense(16, activation='elu',kernel_initializer='he_uniform')
...
class Critic_val(Model): 
    # 评估网络
    def __init__(self,imnet:ImagePro):
        super().__init__() 
        # self.f1 = Dense(256, activation='elu',kernel_initializer='he_uniform')
        self.ima_net = imnet
        self.f31 = Dense(64, activation=None,kernel_initializer='he_uniform')
        self.f4 = Dense(64, activation='elu',kernel_initializer='he_uniform')
        self.f42 = Dense(16, activation='elu',kernel_initializer='he_uniform')
