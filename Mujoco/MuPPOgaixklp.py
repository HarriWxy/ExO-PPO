...
def train_actor(b_s,b_means,b_prob,b_gae,b_a,SIGma):
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        Entr = tf.reduce_mean(0.5+tf.math.log(Sigma*tf.sqrt(2*np.pi)),-1) # Entropy 
        dq3 = tf.sigmoid(2 * (dq1-1)) * 2
        dq2 = tf.multiply(dq3, b_gae)
        Q = 0.01*Entr - dq2 + 0.1*kl_normal(xt,b_means,Sigma,SIGma) # pi * (Entropy -Q) * rate -
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(2e-4,2000,0.99,staircase=True)
