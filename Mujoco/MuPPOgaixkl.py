...
import envpool
from Memo import Memory, kl_normal, action_pro
'EnvPool'
'modified disSAC. Base: Entropy -  pi * V * clip(pi/beta)  // V -> reward'
'value clip, adv clip, adv norm, PPO + kl'
...
def trainNet(istrain, isrender):
    ...
    ep_best=0
    # tensorboard
    train_log_dir='logs/'+ENV_NAME+'/'+datetime.datetime.now().strftime("%m%d-%H%M%S")+'PPOkl'
        ...
        if istrain :
            ...
            for i in range(Train_step):
                ...
                # 获得batch中的每一个变量
                    ...
                    # 更新actor
                    ac_loss = train_actor(b_s,b_mean,b_prob,b_gae,b_a,Sigma)
...
@tf.function
def train_actor(b_s,b_means,b_prob,b_gae,b_a,SIGma):
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        Q = - dq2 + kl_normal(xt,b_means,Sigma,SIGma) # pi * (Entropy -Q) * rate -0.01*Entr
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(3e-4,2000,0.98,staircase=True)
