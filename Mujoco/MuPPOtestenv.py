...
import envpool
from Memo import Memory, kl_normal
'EnvPool'
'modified disSAC. Base:  -  pi * V * clip(pi/beta)  // V -> reward'
...
#                 mem[i].add(s[i],r[i],int(d[i]),log_probs[i],action[i])
...
#         if istrain :
            ...
            #         # 更新actor
            #         ac_loss = train_actor(b_s,b_prob,b_gae,b_a,Sigma)

            # if eps % 8 == 1:  # 一个 eps = 1024 step
                ep_reward, ep_std = evaluate(eps)
                print("ep=",eps,"score=",ep_reward,'+',ep_std)
                # tensorboard
                # with train_sum_writer.as_default():"loss1 = %f" % loss1,"ac-loss = %f" % ac_loss,
...
@tf.function
def train_actor(b_s,b_prob,b_gae,b_a,Sigma):
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        Q =  - dq2 #+ kl_normal(a_temp[0],b_para[:,:,0],a_temp[1],b_para[:,:,1]) # pi * (Entropy -Q) * rate -0.01*Entr
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(2.5e-4,1000,0.92,staircase=True)
    optimizer_cri = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-5))
    lr_schedual = optimizers.schedules.ExponentialDecay(3e-4,1000,0.92,staircase=True)
