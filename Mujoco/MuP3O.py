...
def trainNet(istrain):
    ...
    ep_best=0
    # tensorboard
    train_log_dir='logs/'+ENV_NAME+'/'+datetime.datetime.now().strftime("%m%d-%H%M%S")+'P3O'
...
#============================ 训练网络 ===========================================
        ...
        if istrain :
            ...
            means = np.concatenate([i.means for i in mem],0)
...
def train_actor(b_s,b_means,b_prob,b_gae,b_a):
    batchdist = tfd.Independent(tfd.Normal(b_means[:,0], b_means[:,1]),reinterpreted_batch_ndims=1)
    with tf.GradientTape() as tape: #在这个空间里面计算梯
        ...
        # dq2 = tf.minimum(tf.multiply(dq1, b_gae),clip_adv)tf.minimum(tf.multiply(dq1, b_gae),)
        cliped_dq = tf.sigmoid(2 * (dq1-1)) * 2
        dq2 = tf.multiply(cliped_dq, b_gae)
        Q = - dq2 +  tfd.kl_divergence(batchdist, dist) # 0.1*Entr   kl_normal(a_temp[0],b_para[:,:,0],a_temp[1],b_para[:,:,1]) # pi * (Entropy -Q) * rate -0.01*Entr
...
if __name__ == "__main__":
    ...
    lr_schedual = optimizers.schedules.ExponentialDecay(1.5e-4,6000,0.99,staircase=True)
    optimizer_ac = mixed_precision.LossScaleOptimizer(optimizers.Adam(learning_rate = lr_schedual,epsilon=1e-8)) #
    lr_schedual = optimizers.schedules.ExponentialDecay(2e-4,6000,0.99,staircase=True)
